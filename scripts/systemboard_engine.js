...
var low = 0.0,
  high = 5.0,
  loThreshold = 0.8,
  hiThreshold = 1.4; // from Systeembord manual
function isHigh(x) {
  return x >= hiThreshold;
}
;
function isLow(x) {
  return x < loThreshold;
}
;
function invert(x) {
  return isHigh(x) ? low : high;
}
;
...
function InputNode(x1, y1, isHV=false) {
  ...
  this.eval = function() {
    return (this.child) ? this.child.eval() : false;
  };
  this.isInput = true;
  this.isHV = isHV;
...
function Memory(x1, y1) {
  ...
  this.output = function() {
    if (isHigh(node2.eval()))
      this.nodes[2].state = low;
    if (isHigh(node1.eval()))
      this.nodes[2].state = high; // set always wins
    return true;
  }
  this.remove = function() {};
}
...
function Counter(x1, y1) {
  ...
  this.output = function() {
    ...
    if (isHigh(node6.state) || isHigh(node6.eval())) {
      ...
      if (isLow(currentState) && isHigh(this.state)) {
        this.state = low;
      }
    }
    // update counters
...
function makeLDR(left, top, node) {
  ...
  var imgLDR = new fabric.Image(domLDR, {
    left: left,
    top: top
  });
  imgLDR.scale(0.15);
...
function makeDisplay(x1, y1) {
  ...
  var l = new fabric.Line([x1 + 75, y1 + 30, x1 + 75, y1 + 12], {
    strokeWidth: 2,
    stroke: 'red',
    selectable: false,
    evented: false
...
function Voltmeter(x1, y1) {
  ...
  this.display = new fabric.Line([x1 + 22, y1 + 22, x1 + 22, y1 + 4], {
    strokeWidth: 2,
    stroke: 'red',
    selectable: false,
    evented: false
